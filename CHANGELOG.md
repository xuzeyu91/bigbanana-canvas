...
+ [新增] 画布图片生成支持可选的透明背景请求，已选画布元素支持打包导出。
+ [优化] 对话 Agent 默认文本模型调整为 `gpt-5.6-terra`、`gpt-5.6-sol`、`gpt-5.4`、`gpt-5.5` 等，移除 `gpt-5.1`、`gpt-5.2`，并降低长流式回复的界面更新频率。
+ [新增] 渠道兼容Gemini格式。
+ [优化] 模型请求通过同源 new-api 代理转发到 `api.antsk.cn`，支持流式和媒体响应透传。
+ [新增] 画布新增可选的制作工作流：语义资产、镜头分组、生成前检查、锁定参考注入和节点生成记录。
+ [优化] 视频设置仅按所选模型展示并提交支持的比例与时长，不再提供或提交用户可配置的分辨率参数。
+ [优化] 图片设置会按模型约束分辨率、比例和输出尺寸，并支持 Gemini 与 GPT Image 2 的 2K/4K 生成。
