...
* **多模态 AI 生成 (Multimodal Generation Flow)**
  浏览器前端使用本地保存的 API Key 调用同源 new-api 代理，再由服务端转发到 AntSK API 网关，完美兼容主流的大语言模型（如 GPT-5.5、GPT-5.2、Claude 4.6 Sonnet）、图像模型（如 Gemini 3.1 Flash Image、GPT Image 1.5）、音频模型（如 GPT Audio Mini）与视频模型（如 HappyHorse 1.0）。支持多参考图、多参考视频、多参考音频输入，完美支持 Seedance 2.0 异步视频生成工作流。
...
* **提示词库与本地素材 (Prompt & Asset Center)**
  Next.js 路由从 [GPT-Image-2 Prompt Gallery](https://image2.tree456.com/) 自动拉取并内存缓存带有完整提示词和可用封面图的案例。支持按标题、标签与分类筛选，可一键复制或将其加入“我的素材”中。内置本地素材库，支持保存、编辑、分类和直接拖入画布。
...
推荐使用 Vercel 直接导入本项目仓库，项目根目录下已经完整配置了 `vercel.json`，会自动处理 `web/` 的前端构建。在整个执行链中，用户的 AI 密钥和画布项目均保存在浏览器本地，模型请求会通过同源 new-api 代理转发到 AntSK API 网关；画布数据也可以使用 WebDAV 自行进行云同步。
...
2. 在模型配置通道中填写你的 API Key 和模型名；Base URL 固定为同源 new-api 代理，会由服务端转发到 AntSK API 网关。
