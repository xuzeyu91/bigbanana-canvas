...
export function CanvasNodeSplitDialog({ dataUrl, open, onClose, onConfirm }: { dataUrl: string; open: boolean; onClose: () => void; onConfirm: (params: CanvasImageSplitParams) => void }) {
    ...
    const update = (key: "rows" | "columns", value: string | number | null) => {
        setParams((current) => {
            const count = clampGrid(value ?? current[key]);
            const lineKey = key === "rows" ? "horizontalLines" : "verticalLines";
            return { ...current, [key]: count, [lineKey]: buildGridLines(count) };
        });
    };
    const addLine = (axis: "horizontal" | "vertical") => {
        setParams((current) => {
            const key = axis === "horizontal" ? "horizontalLines" : "verticalLines";
            const lines = [...(current[key] || []), findLineSpot(current[key] || [])].sort((left, right) => left - right);
    ...
    const startDrag = (axis: "horizontal" | "vertical", index: number, event: ReactPointerEvent) => {
        ...
        const move = (moveEvent: PointerEvent) => {
            ...
            setLine(axis, index, axis === "horizontal" ? (moveEvent.clientY - box.top) / box.height : (moveEvent.clientX - box.left) / box.width);
        };
        const stop = () => {
...
function findLineSpot(lines: number[]) {
    const cuts = [0, ...lines, 1].sort((left, right) => left - right);
    let spot = 0.5;
    let maxGap = 0;
...
function clampLine(value: number, min: number, max: number) {
    const margin = Math.min(0.01, Math.max(0, (max - min) / 2));
